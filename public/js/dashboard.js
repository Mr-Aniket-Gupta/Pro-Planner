...
async function fetchProjects() {
    ...
    try {
        ...
        if (projects.length > 0) {
            ...
            await renderProjectDetails(); // Project details show karo
            await fetchTasks(projects[0]._id); // Us project ke tasks fetch karo
        } else {
            // Agar koi project nahi hai toh empty state show karo
...
async function markProjectComplete(projectId) {
    ...
    if (res.ok) {
        ...
        const updatedProject = await (await fetch(`/api/projects/${projectId}/details`)).json();
        projects[selectedProjectIndex] = { ...project, ...updatedProject.project };
        await renderProjectDetails();
...
async function exportProjectPDF(projectId) {
    try {
        ...
        // Helper: safely write text, adding new page when needed
        function addText(text, x, yPos, options = {}, lineHeight = 14) {
        ...
        doc.text('PROJECT REPORT', doc.internal.pageSize.width / 2, y, { align: 'center' });
        y += 30;
        ...
        doc.setFontSize(12);
        y = addText(project.name || 'N/A', leftMargin + 10, y);

        // Project Description
        y += 20;
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(16);
        doc.text('Project Description:', leftMargin, y);
        y += 18;
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(12);
        y = addText(project.desc || 'No description available', leftMargin + 10, y);

        // Features
        const hasBasic = project.basic && project.basic.length;
        const hasAdvanced = project.advanced && project.advanced.length;

        if (hasBasic || hasAdvanced) {
            y += 25;
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(16);
            doc.text('Project Features:', leftMargin, y);
            y += 20;
            ...
            if (hasBasic) {
                ...
                doc.setFontSize(13);
                doc.setTextColor(30, 64, 175);
                doc.text('Basic Features:', leftMargin, y);
                y += 15;
                doc.setFont('helvetica', 'normal');
                doc.setFontSize(11);
                doc.setTextColor(0, 0, 0);

                for (const feature of project.basic) {
                    y = addText(`• ${feature}`, leftMargin + 10, y);
                }
            }
            ...
            if (hasAdvanced) {
                ...
                doc.setFontSize(13);
                doc.setTextColor(30, 64, 175);
                doc.text('Advanced Features:', leftMargin, y);
                y += 15;
                doc.setFont('helvetica', 'normal');
                doc.setFontSize(11);
                doc.setTextColor(0, 0, 0);

                for (const feature of project.advanced) {
                    y = addText(`• ${feature}`, leftMargin + 10, y);
                }
            }
        }
        ...
        doc.setFontSize(16);
        doc.setTextColor(30, 64, 175);
        doc.text('Project Progress:', leftMargin, y);
        y += 18;
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(12);
        y = addText(`${progress}% Complete`, leftMargin + 10, y);
        ...
        if (tasks.length > 0) {
            ...
            doc.setFontSize(16);
            doc.setTextColor(30, 64, 175);
            doc.text('Project Tasks:', leftMargin, y);
            y += 20;

            const headers = ['Task Name', 'Tag', 'Priority', 'Due Date', 'Status'];
            const colWidths = [0.35, 0.2, 0.15, 0.15, 0.15].map(f => usableWidth * f);
            const rowHeight = 16;

            function drawTableRow(values, bold = false, fill = false) {
                ...
                if (fill) {
                    ...
                    doc.rect(leftMargin, y - 10, usableWidth, rowHeight, 'F');
                }
                ...
                values.forEach((val, i) => {
                    const textLines = doc.splitTextToSize(val.toString(), colWidths[i] - 10);
                    ...
                    x += colWidths[i];
                    y -= 10 * textLines.length; // reset after writing lines
                });
                y += rowHeight;
            }
            ...
            tasks.forEach((t, i) => {
                const priorityColor =
                    t.priority === 'High' ? [220, 38, 38] :
                        t.priority === 'Medium' ? [245, 158, 11] :
                            [59, 130, 246];
        ...
        if (project.notes && project.notes.trim()) {
            ...
            doc.setFontSize(16);
            doc.setTextColor(30, 64, 175);
            doc.text('Quick Notes:', leftMargin, y);
            y += 18;
            doc.setFont('helvetica', 'normal');
            doc.setFontSize(12);
            doc.setTextColor(0, 0, 0);
            y = addText(project.notes, leftMargin + 10, y);
        }
        ...
        doc.text(`Generated on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}`, doc.internal.pageSize.width / 2, pageHeight - 40, { align: 'center' });
        doc.text('ProPlanner - Professional Project Management', doc.internal.pageSize.width / 2, pageHeight - 25, { align: 'center' });
...
async function pollForProjectUpdates() {
    ...
    try {
        ...
        console.error("Project polling failed:", err);
    }
}
...
function renderTasks(filter = '', tag = '', priority = '') {
    ...
    const taskOverviewHeading = document.getElementById('taskOverviewHeading');
    ...
    if (taskOverviewHeading) {
        if (selectedProjectIndex !== null && projects && projects[selectedProjectIndex]) {
            const projectName = projects[selectedProjectIndex].name;
            taskOverviewHeading.textContent = `Task Overview - ${projectName}`;
        } else {
            taskOverviewHeading.textContent = 'Task Overview';
        }
    }
    ...
    if (emptyTaskState) {
        emptyTaskState.style.display = filtered.length === 0 ? '' : 'none';
    }
...
async function updateTaskInDB(taskId, data, projectId) {
    ...
    try {
        ...
        if (data.completed !== undefined) {
            ...
            const taskText = data.text || task?.text || taskId;
            addActivityFeed(`Task "${taskText}" ${status}`, 'task');
        } else {
            const task = taskList.find(t => t._id === taskId);
            const taskText = data.text || task?.text || taskId;
            addActivityFeed(`Task "${taskText}" details updated`, 'update');
        }
        showAlert({ icon: 'success', title: 'Task updated!' });

        const taskIndex = taskList.findIndex(t => t._id === taskId);
        if (taskIndex !== -1) {
            taskList[taskIndex] = { ...taskList[taskIndex], ...data };
        }

        const filter = document.getElementById('taskSearch')?.value || '';
        const tag = document.getElementById('taskTagFilter')?.value || '';
        const priority = document.getElementById('taskPriorityFilter')?.value || '';

        renderTasks(filter, tag, priority);
...
function updateSummaryCards() {
    ...
    document.getElementById('summaryTotalTasks').innerText = totalTasks;
    document.getElementById('summaryCompletedTasks').innerText = completedTasks;
    document.getElementById('summaryPendingTasks').innerText = pendingTasks;
    document.getElementById('summaryHighPriorityTasks').innerText = highPriorityTasks;
}
...
function addActivityFeed(msg, type = 'info') {
    ...
    if (activityFeedArr.length > 50) activityFeedArr.pop();
...
function updateActivityFeed() {
    ...
    activityFeedArr.forEach(a => {
        const li = document.createElement('li');
        li.className = 'flex items-center justify-between py-2 px-3 rounded-lg hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors';
        ...
        li.innerHTML = `
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm text-gray-700 dark:text-gray-300">${a.msg}</span>
            </div>
            <span class="text-xs text-gray-400 dark:text-gray-500 font-medium">${formatRelativeTime(a.time)}</span>
        `;
        ul.appendChild(li);
    });
}
...
function clearActivityFeed() {
    Swal.fire({
        ...
        customClass: {
            popup: 'rounded-2xl',
            confirmButton: 'bg-red-500 hover:bg-red-600 px-4 py-2 rounded-xl',
            cancelButton: 'bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-xl'
        }
    }).then((result) => {
        if (result.isConfirmed) {
            activityFeedArr = [];
            localStorage.removeItem('proplanner_activity_feed');
            updateActivityFeed();
            Toast.fire({
                icon: 'success',
...
// इवेंट लिसनर जोड़ें
document.getElementById('clearActivityFeedBtn').addEventListener('click', clearActivityFeed);
...
function initializeDragAndDrop() {
    if (window.Sortable) {
        ...
        setupDoubleClickDragActivation(projectsSortable, 'projectList', 'projects');
        setupDoubleClickDragActivation(tasksSortable, 'taskList', 'tasks');
        setupDoubleClickDragActivation(todosSortable, 'sidebarTodoList', 'todos');

        document.addEventListener('click', function (e) {
            if (!e.target.closest('#projectList li') && !e.target.closest('#taskList li') && !e.target.closest('#sidebarTodoList li')) {
                [projectsSortable, tasksSortable, todosSortable].forEach(sortable => {
...
function setupDoubleClickDragActivation(sortable, listId, type) {
    document.getElementById(listId).addEventListener('dblclick', function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (e.target.closest('li')) {
            ...
            showDragModeNotification(`Drag mode enabled - Drag to reorder ${type}`, 'success');
...
async function initializeApplication() {
    ...
    setInterval(pollForProjectUpdates, 15000);
}
